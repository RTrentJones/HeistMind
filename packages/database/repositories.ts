...
export interface GamePlayerRepository {
    addPlayer(gameId: string, playerId: string, invitedBy: string, role?: GameRole): Promise<Result<GamePlayer>>
    removePlayer(gameId: string, playerId: string, removedBy: string): Promise<Result<void>>
    updateRole(gameId: string, playerId: string, newRole: GameRole, updatedBy: string): Promise<Result<GamePlayer>>
...
export interface CharacterRepository {
    ...
    delete(id: string, userId: string): Promise<Result<void>>
    addExperience(id: string, userId: string, amount: number, reason: string): Promise<Result<Character>>
    transferToGame(characterId: string, targetGameId: string, userId: string): Promise<Result<Character>>
    cloneCharacter(characterId: string, targetGameId: string, userId: string): Promise<Result<Character>>
}
...
export interface GameManagementRepository {
    createGameWithRuleset(userId: string, gameData: CreateGameData, rulesetData?: CreateRulesetData): Promise<Result<GameWithDetails>>
...
export interface CharacterManagementRepository {
    createCharacterWithValidation(userId: string, data: CreateCharacterData): Promise<Result<CharacterWithDetails>>
    advanceCharacter(characterId: string, userId: string, advancementData: CharacterAdvancement): Promise<Result<Character>>
...
export interface Activity {
    id: string
    type: 'game_created' | 'player_joined' | 'character_created' | 'character_advanced' | 'invitation_sent'
    description: string
    userId: string
    gameId?: string
    characterId?: string
...
export interface RepositoryFactory {
    ...
    createWithTransaction(): Promise<DatabaseRepositories & { commit(): Promise<void>; rollback(): Promise<void> }>
}
