...
export class SupabaseAuthService implements AuthService {
    ...
    async linkDiscordAccount(userId: string, discordData: DiscordAuthData): Promise<AuthResponse<User>> {
        try {
    ...
    async getUserByDiscordId(discordId: string): Promise<User | null> {
        try {
            ...
            // Get full user data
            const { data: { user }, error: userError } = await this.supabase.auth.admin.getUserById(data.id)
