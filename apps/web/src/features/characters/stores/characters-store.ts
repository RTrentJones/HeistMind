        ...
        persist(
            (set, get) => ({
                ...
                updateCharacter: async (characterId: string, data: UpdateCharacterData) => {
                    ...
                    try {
                        const repositories = getRepositories()
                        const result = await repositories.characters.update(characterId, 'current-user-id', data) // TODO: Get from auth
                        ...
                        set((state) => ({
                            characters: state.characters.map(char => char.id === characterId ? updatedCharacter : char),
                            userCharacters: state.userCharacters.map(char => char.id === characterId ? updatedCharacter : char),
                            ...
                            characterDetails: state.characterDetails[characterId]
                                ? { ...state.characterDetails, [characterId]: { ...state.characterDetails[characterId], ...updatedCharacter } }
                                : state.characterDetails,
                ...
                addExperience: async (characterId: string, amount: number, reason: string) => {
                    ...
                    try {
                        const repositories = getRepositories()
                        const result = await repositories.characters.addExperience(characterId, 'current-user-id', amount, reason) // TODO: Get from auth
                ...
                advanceCharacter: async (characterId: string, advancement: CharacterAdvancement) => {
                    ...
                    try {
                        ...
                        const repositories = getRepositories()
                        const result = await repositories.characterManagement.advanceCharacter(characterId, 'current-user-id', advancement) // TODO: Get from auth
                ...
                transferCharacter: async (characterId: string, targetGameId: string) => {
                    ...
                    try {
                        const repositories = getRepositories()
                        const result = await repositories.characters.transferToGame(characterId, targetGameId, 'current-user-id') // TODO: Get from auth
                        ...
                        set((state) => ({
                            characters: state.characters.map(char => char.id === characterId ? transferredCharacter : char),
                            userCharacters: state.userCharacters.map(char => char.id === characterId ? transferredCharacter : char),
                            gameCharacters: Object.fromEntries(
                                Object.entries(state.gameCharacters).map(([gameId, chars]) => [
                                    gameId,
                            ...
                            characterDetails: state.characterDetails[characterId]
                                ? { ...state.characterDetails, [characterId]: { ...state.characterDetails[characterId], ...transferredCharacter } }
                                : state.characterDetails,
                            selectedCharacter: state.selectedCharacter?.id === characterId
                ...
                cloneCharacter: async (characterId: string, targetGameId: string) => {
                    ...
                    try {
                        const repositories = getRepositories()
                        const result = await repositories.characters.cloneCharacter(characterId, targetGameId, 'current-user-id') // TODO: Get from auth
...
export const useCharacters = () => useCharactersStore((state) => ({
...
export const useCharacterActions = () => useCharactersStore((state) => ({
...
// Computed selectors
export const useActiveCharacters = () => useCharactersStore((state) =>
    state.characters.filter(char => char.status === 'active')
)

export const useGameCharacters = (gameId: string) => useCharactersStore((state) =>
    state.gameCharacters[gameId] || []
)
