...
export class ErrorHandler {
  ...
  private handleValidationError(
    ...
    const message =
      firstError?.message || this.translate?.('errors.validation.general') || error.message;
  ...
  public async handleAsync<T>(operation: () => Promise<T>, context?: string): Promise<T | null> {
    try {
  ...
  public handleSync<T>(operation: () => T, context?: string): T | null {
    try {
      return operation();
    } catch (error) {
      this.handle(error, context);
